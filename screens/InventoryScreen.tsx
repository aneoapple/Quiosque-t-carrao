        ...
        <div className="pb-20">
            ...
            {(modalType === 'ENTRY' || modalType === 'EXIT' || modalType === 'ADJUST') && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white p-6 rounded-3xl w-full max-w-sm shadow-2xl animate-fade-in relative">
                        <button onClick={resetModals} className="absolute top-4 right-4 text-slate-300 hover:text-slate-500 rounded-full p-1 hover:bg-slate-100"><X size={20} /></button>

                        <h3 className="font-black mb-6 text-slate-800 text-center text-xl tracking-tight">
                            {modalType === 'ENTRY' ? 'Entrada de Estoque' : modalType === 'EXIT' ? 'Saída de Estoque' : 'Ajuste de Balanço'}
                        </h3>

                        {/* SELEÇÃO DE PRODUTO SE NECESSÁRIO */}
                        <div className="mb-4">
                            {!selectedProduct ? (
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-slate-400 uppercase ml-1">Selecionar Produto</label>
                                    <select
                                        className="w-full p-3 border border-slate-200 rounded-xl font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50"
                                ...
                                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-center gap-3">
                                    <div className="w-12 h-12 bg-white rounded-xl border border-slate-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                                        {products.find(p => p.id === selectedProduct)?.image_url ? (
                                            <img src={products.find(p => p.id === selectedProduct)?.image_url} className="w-full h-full object-cover" />
                                        ) : <ImageIcon size={20} className="text-slate-300" />}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="font-bold text-slate-800 text-sm truncate">{products.find(p => p.id === selectedProduct)?.name}</p>
                                        <p className="text-xs text-slate-500 font-bold mt-0.5">
                                            Estoque Atual: <span className="text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded-md">{getProductStock(selectedProduct)} {products.find(p => p.id === selectedProduct)?.unit}</span>
                                        </p>
                                    </div>
                                    {/* Botão para trocar produto se veio pelo botão geral (lógica pode ser refinada, mas por agora mantém locked se já selecionado via row action) */}
                                </div>
                            )}
                        </div>

                        {/* INPUTS - Só aparecem se tiver produto selecionado */}
                        {selectedProduct && (
                            <div className="animate-fade-in space-y-4">
                                {modalType !== 'ADJUST' && (
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className="w-full border border-slate-200 p-3 rounded-xl font-bold text-slate-700 bg-slate-50 outline-none focus:ring-2 focus:ring-indigo-500"
                                        >
                                            {modalType === 'ENTRY' ? (
                                                <>
                                                    <option value={MovementType.ENTRADA_COMPRA}>Compra de Fornecedor</option>
                                                    <option value={MovementType.PRODUCAO}>Produção Interna</option>
                                                    <option value={MovementType.ENTRADA_AJUSTE}>Ajuste (Entrada)</option>
                                                </>
                                            ) : (
                                                <>
                                                    <option value={MovementType.SAIDA_PERDA}>Perda / Validade / Quebra</option>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">
                                        {modalType === 'ADJUST'
                                            ? 'Nova Quantidade (Total Físico)'
                                            : (modalType === 'ENTRY' ? 'Quantidade a Adicionar' : 'Quantidade a Remover')
                                        }
                                    </label>
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full border border-slate-200 p-4 rounded-xl text-2xl font-black text-center text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                        />
                                    ...
                                    {/* Helper para Ajuste */}
                                    {modalType === 'ADJUST' && qtyInput && (
                                        <div className={`mt-2 text-center text-xs font-bold px-3 py-2 rounded-lg ${parseInt(qtyInput) - getProductStock(selectedProduct) === 0 ? 'bg-slate-100 text-slate-500' : (parseInt(qtyInput) - getProductStock(selectedProduct) > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700')}`}>
                                            Diferença Resultante: {' '}
                                            {parseInt(qtyInput) - getProductStock(selectedProduct) > 0 ? '+' : ''}
                                            {parseInt(qtyInput) - getProductStock(selectedProduct)} {products.find(p => p.id === selectedProduct)?.unit}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        placeholder="Ex: Contagem mensal, Nota Fiscal..."
                                        value={obsInput}
                                        onChange={e => setObsInput(e.target.value)}
                                        className="w-full border border-slate-200 p-3 rounded-xl text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 font-medium"
                                    />
                                </div>

                                <div className="pt-2">
                                    <button
                                        onClick={modalType === 'ADJUST' ? handleSaveAdjustment : handleSaveMovement}
                                        className={`w-full py-4 rounded-xl font-bold text-white shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2 ${modalType === 'EXIT' ? 'bg-red-500 hover:bg-red-600 shadow-red-200' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200'}`}
                                    >
                                        <Save size={20} /> Confirmar {modalType === 'ADJUST' ? 'Ajuste' : 'Movimentação'}
                                    </button>
                                </div>
                            </div>
                        )}

...
const PosScreen: React.FC<PosScreenProps> = ({ onBack }) => {
  ...
  const processSale = async (method: string) => {
    ...
    // Passa selectedEmployeeId se houver
    const saleId = await createSale(saleItems, method, 0, isConsumption, selectedEmployeeId);
    ...
    if (method === PaymentMethod.FUNCIONARIO && selectedEmployeeId) {
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              {employees.filter(e => e.active !== false).length === 0 ? (
                <div className="text-center py-6 text-slate-400">
                  ...
                  <p className="text-xs mt-1">Cadastre em Gestão de Equipe.</p>
                </div>
              ) : (
